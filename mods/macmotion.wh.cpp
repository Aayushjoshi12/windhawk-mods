    ...
    if (IsExplorerProcess() &&
        ...
        if (SUCCEEDED(DwmGetWindowAttribute(
            ...
            if (preCloaked == hWnd &&
                ...
                if (!StartAnimation(hWnd, TRUE, originalExStyle, TRUE, FALSE, 0)) {
                    SetWindowCloak(hWnd, FALSE);
                    UpdateDwmTransitions(hWnd, TRUE);
...
DWORD WINAPI NativeAltTabCommitThread(LPVOID parameter) {
    ...
    if (SUCCEEDED(DwmGetWindowAttribute(
        ...
        if (preCloaked == target &&
            ...
            if (!StartAnimation(target, TRUE, originalExStyle, TRUE, FALSE, 0)) {
                SetWindowCloak(target, FALSE);
                UpdateDwmTransitions(target, TRUE);
            }
            return 0;
...
bool StartAnimation(HWND hWnd, BOOL rising, LONG_PTR originalExStyle, BOOL cloakHidden = FALSE, BOOL isClosing = FALSE, UINT closeMsg = 0) {
...
static bool RunCloseAnimation(HWND hWnd, UINT closeMsg) {
    ...
    const bool started = StartAnimation(
        hWnd, FALSE, GetWindowLongPtrW(hWnd, GWL_EXSTYLE), FALSE, TRUE,
        closeMsg);
